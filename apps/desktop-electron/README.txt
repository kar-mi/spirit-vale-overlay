Spirit Vale Overlay (Electron build)

Extract the complete ZIP, then run "Spirit Vale Overlay.exe" on Windows x64.
Npcap is required. Install it in WinPcap API-compatible mode without restricting it to administrators.

This is the Electron shell: a Chromium-based alternative to the default build. Reach
for it when the default build has WebView2 install problems or window/style glitches.
It is larger to download and behaves identically otherwise.
