...
void desktopView.rpc?.request.getState({}).then((next) => { const repaired = repairRendererPayload(next); state.value = repaired; document.title = repaired.title; });
    ...
    <div class="picker-shell">
      ...
      <main>
        <div class="picker-intro">
          ...
          <button class="btn" type="button" onClick={() => desktopView.rpc?.send.refresh({})}>Refresh</button>
        </div>
        ...
        <div class="picker-actions">
          <button id="choose-file-button" class="btn btn-ghost" type="button" onClick={() => desktopView.rpc?.send.chooseFile({})}>Choose JSON file…</button>
          {next.canOpenLogFolder && (
            <button class="btn btn-ghost" type="button" onClick={() => desktopView.rpc?.send.openLogFolder({})}>Open log folder</button>
          )}
