  ...
  return <main class="app-shell">
    ...
    <section class="settings-content">
      ...
      <section class="settings-panel" hidden={tab !== "overlay"}>
        ...
        <label class="settings-check"><input type="checkbox" checked={overlay.autoHideWhenUnfocused} disabled={busy} onChange={(event) => update(electroview.rpc?.request.setAutoHideWhenUnfocused({ enabled: event.currentTarget.checked }))} /><span>Auto-hide when neither the game nor this app is focused</span></label>
        <p class="settings-hint">Hides the overlay when you switch to another window (e.g. File Explorer or a browser). Switching back to the game, or to this app's own windows (like this Settings window), shows it again. A manual hide (via {overlay.shortcuts.toggleOverlayVisible} or the button above) always takes priority — it won't be undone by switching windows until you show it manually again.</p>
