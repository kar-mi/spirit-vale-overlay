    ...
    items: [
      ...
      }] : []),
      {
        ...
        content: <><div class="settings-card"><h2>Visible elements</h2>{OVERLAY_ELEMENT_IDS.map((id) => {
          // The minimap answers to its own master switch; this row cannot resurrect a disabled feature.
        ...
        })}</div>{!overlay.minimapEnabled && <p class="settings-hint">The minimap is turned off in the Minimap section; enable it there to use this row.</p>}<p class="settings-hint">{overlay.personalName ? `Detected character: ${overlay.personalName}` : "Waiting to detect your active character."}</p></>,
      },
    ],
  };
}
